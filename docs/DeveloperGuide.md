...
- Improve validation for commands with no parameters: Currently, commands like exit, list, and clear will still execute even if the user types words behind (e.g., exit please). We plan to enhance the parser to strictly reject any extra arguments for these commands, displaying an invalid format error.
